...
def classify_images(images_dir, results_dic, model):
    ...
    pet_labels_dic = results_dic #results_dic from get_pet_labels function
    filenames=list(pet_labels_dic.keys())
    pet_labels=list(pet_labels_dic.values())
    ...
    ### Populate classifier_labels list
    ############# Modification based on Code Review ############
    #for key, pet_label in results_dic.items():for key, pet_label in results_dic.items():
    for key in results_dic:
    ############################################################
    #for idx in range (0, len(filenames), 1):
        # Defines a dog test image from pet_images folder using the full path
        ...
        ############# Modification based on Code Review ############
        #####Converting output to lower case and strip whitespace##########
        #Set strings to lower case
        low_image_classif = image_classification.lower()
        # Strip off starting/trailing whitespace characters
        imag_classif = low_image_classif.strip()

        classifier_labels.append(imag_classif)


    #Updating the results_dic with index 1 and index 2
    for idx in range (0, len(filenames), 1):
    ####### adding index 1= Classifier Label to results_dic  ##############
        results_dic[filenames[idx]] = [ pet_labels[idx][0], classifier_labels[idx] ]
    ...
    ##### Final Result is results_dic updated with index 1 and index 2
